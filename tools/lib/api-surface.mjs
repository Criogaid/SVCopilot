...
const CAPTURE_KEYS = ["kind", "schemaVersion", "capturedAt", "host", "probe", "classes"];
const HOST_KEYS = ["product", "version", "platform", "versionNumber"];
const PROBE_KEYS = ["name", "version", "readOnly", "trialCalls"];
...
const TRIAL_KEYS = ["status", "returnedType", "errorKind"];
...
export function normalizeApiSurfaceCapture(raw) {
  ...
  if (capture.schemaVersion !== API_SURFACE_SCHEMA_VERSION) {
    throw surfaceError(`capture.schemaVersion must be ${API_SURFACE_SCHEMA_VERSION}`);
  }
  ...
  for (const [className, bucket] of [...observed.entries()].sort(compareStrings)) {
    ...
    for (const [memberName, member] of [...bucket.members.entries()].sort(compareStrings)) {
      ...
      undocumented.push({
        ...
        ...(member.trial ? { trial: member.trial } : {}),
      });
    }
  }
