...
| `sv_plan_pitch_gesture` | 无副作用音高手势规划器（纯内存只读，不写宿主）：把 transition/attack/release/vibrato 意图（秒/quarter/音符比例经 TimeAxis 转整数 BLICK）编译成有界不超调的 PitchControlCurve add 计划，anchor 音高默认取音符目标音（group-relative semitone），深度/频率/相位有显式范围与 clamp 警告；只 add 自有对象，绝不删除/覆盖既有 PitchControl。产出统一 `apply` 信封（`sv_patch_pitch_controls`，携带 `expectedNotes` 等漂移守卫，旧 `applyRequests` 保留一版并标注 deprecated）；短音/相邻重叠/休止/非零 offset 均有边界处理。能否更好听 human_only |
