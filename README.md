...
| `sv_snapshot_range` | 一次读取可编辑范围上下文：相交音符/occurrence 身份、voice、Automation、computed pitch、attributes、processing、tempo/meter、mixer、PitchControl（`include:["pitchControls"]`：Point/Curve discriminator、group-local 与 occurrence-absolute 双坐标、ownership 与 fingerprint）；严格拒绝未知字段；按数据类型预算分页，cursor 续读不重访宿主；`sinceToken` 命中时用 `detailCursor` 取得新 context 身份 |
...
| `sv_patch_pitch_controls` | 在一个 Undo 中对单个 NoteGroup occurrence 增删改 PitchControlPoint/Curve（SynthV 2.1+）：按 `expectedFingerprint` 唯一解析目标（零匹配 `UNKNOWN_CONTROL`、多匹配 `AMBIGUOUS_CONTROL`，绝不取第一个；add/remove 触发宿主重排，故一律用执行时 `getIndexInParent` 而非旧索引），写前复核 group UUID、全组 fingerprint（`expectedPitchControlFingerprint`）与 `expectedNotes`/`expectedTimeOffsetBlick`/`expectedPitchOffsetSemitone` 漂移守卫，shared-target commit 前扫描整个工程需 `allowSharedTargetMutation:true`；journal 在第一次写之前 clone + 抓取完整 scriptData（含外部脚本值，不调 `clearScriptData`），update 走原位 set；新建对象写入 `svcopilot.*` 持久 ownership ID。单位严格：position=group-local 整数 BLICK，pitch=group-relative semitone，Curve 点相对 anchor——绝不与 pitchDelta 的 cents 混用。dryRun/no_change 零写零 Undo；失败逆序补偿并读回，诚实报 `rolled_back`/`rollback_failed`/`partial`/`outcome_unknown` |
| `sv_plan_pitch_gesture` | 无副作用音高变化规划器（纯内存只读，不写宿主）：把 transition/attack/release/vibrato 意图（秒/quarter/音符比例经 TimeAxis 转整数 BLICK）编译成有界不超调的 PitchControlCurve add 计划。官方 V2 精确 `br` 默认为非旋律事件，`specialEventPolicy:"warn_and_skip"` 会结构化跳过，`include` 才显式纳入，`error` 则整份拒绝；全跳过返回 no_change、零 apply/Undo。anchor 音高默认取音符目标音（group-relative semitone），深度/频率/相位有显式范围与 clamp 警告；只 add 自有对象，绝不删除/覆盖既有 PitchControl。产出统一 `apply` 信封（`sv_patch_pitch_controls`，携带 `expectedNotes` 等漂移守卫，旧 `applyRequests` 保留一版并标注 deprecated）；短音/相邻重叠/休止/非零 offset 均有边界处理。能否更好听 human_only |
| `sv_bake_computed_pitch` | 把宿主 computed pitch 固化为一条自有 PitchControlCurve（绝对 MIDI → group-relative anchor + 点偏移，group-local BLICK）：RDP 有界简化保留端点与有效区间边界并报告 `fitError`/`coverage`/`finiteFrames`/`nullFrames`/`sourceSampling`；覆盖不足/全 null/空数组一律零写入（`INSUFFICIENT_COMPUTED_PITCH`，全 null 绝不解释为零音高）。策略 `preserve_existing`（只新增）/`replace_owned`（替换范围内自有对象）/`replace_explicit`（只替换调用者逐对象确认者）；既有 pitchDelta 默认保留，跨数据面清除（需跨类型事务）本版显式拒绝（`PITCH_DELTA_CLEAR_UNSUPPORTED`）。写面复用 `sv_patch_pitch_controls` 事务核（一个 Undo、读回、逆序补偿） |
| `sv_compare_computed_pitch` | 对 `sv_snapshot_range` 已存的 computed pitch 做客观演唱分析（纯内存只读，不访问宿主）：`compare_to_target` 报告逐音符稳态段 `centerErrorCent`、逐帧诊断、去趋势自相关颤音 rate/depth/regularity、转换 overshoot/arrival/settling 与异常区段；官方特殊歌词状态机排除精确 `br` 等非旋律事件及其帧，`inputNoteCount`/`melodicNoteCount`/`excludedEvents` 与 `eligibleFrameCount` 明示分母，纯呼吸范围报 `NO_MELODIC_EVIDENCE` 而非 processing pending。`compare_contexts` 在同一采样栅格上按乐谱位置逐帧对比调前/调后（after−before），并给出 §13.8 风格半音差值统计——Hz 类指标按各侧自己的 tempo 图换算帧率；逐音符对按乐谱位置匹配，结构编辑后无法与未变 before 音符配对的标 `unmatched` 并省略 delta（`PER_NOTE_UNMATCHED`），不做误导性跨音符对比；matched/unmatched 与 count 同为全量计数，明细截断只影响 items。`anomalySegments.items` 默认按 `startBlick` 乐谱时间序返回并显式声明 `sortBy`（`anomalySortBy:"severity"` 才按峰值降序），`top` 恒为最严重段——时间序截断不吞掉最严重证据；有效帧覆盖率低于 `analysis.lowCoverageWarnRatio`（默认 0.5）时发 `LOW_COMPUTED_PITCH_COVERAGE`，低样本统计不冒充可靠结论。null 帧保留不进统计；帧率对颤音的适配度分级 ok/borderline/too_coarse 而不是硬拒；全部阈值为未经真机校准的工程默认值，听感判断仍属人类 |
| `sv_plan_expression` | dry-run 演唱表现规划器（纯内存只读，不写宿主）：显式表现手法（scoop/fall/portamento/vibrato/hairpin，quarter 音乐时间参数）与小词表意图（jpop、controlled_belt/soft_airy/light_rasp、cool_anger/tender、段落）编译为单位显式（cents/dB/x/±1，writeSurface=automation）的可审阅计划，同参数重叠表现手法求和、基线守卫、约束 clamp 与点数预算；section/emotion 单独出现时播种逐乐句基线动态/音色弧线（低置信启发式）而不再 EMPTY_PLAN；`intent.preset`（jpop_cool/jpop_belt/controlled_anger/intimate_whisper/spoken_rap_transition）是可审阅的常量展开——展开字段/约束默认值逐项回显在 `presetExpansion`，用户显式 intent 字段覆盖 preset 同名值（`PRESET_FIELD_OVERRIDDEN`）、显式 constraints 永远优先，spoken_rap_transition 额外在长音上播种 vibratoEnv 压平处理，绝非黑箱按钮；产出统一 `apply` 交接信封（读 `apply.tool` + `apply.arguments` 逐字提交；同参数多簇时按序分成 K 次调用，`apply.additionalCalls` 按序列出、`apply.expectedUserUndoSteps` 如实等于 K，且声明它们是独立事务；旧 `applyRequests` 保留一版并标注 deprecated），target 携带表现手法锚点音符的 `expectedNotes` 完整指纹与快照时 `expectedTimeOffsetBlick`——apply 预检逐条复核，音符被改动或整个 reference 被 setTimeOffset 移动均判 STALE_CONTEXT，而非在错误位置写曲线。意图派生复用特殊歌词状态机，精确 `br`、orphan `+/-` 与单独 apostrophe 不作为旋律锚点并返回 `NON_MELODIC_SPECIAL_EVENT_SKIPPED`；合法 continuation 链仍参与，显式点名始终尊重调用者。replace 覆盖区间内既有点且规划器不读宿主（review 显式声明）；natural vibrato 不可观测；意图映射为启发式；是否更好听 human_only |
| `sv_align_lyrics` | 无副作用咬字规划器（纯内存只读）：混排歌词分词并铺排到音符——日语假名确定性 mora 切分（拗音并前拍、促音/拨音/长音各占一拍）、英语元音簇音节启发式（词 + "+" 续拍，~85-90% 文献准确率；显式 `+/-` 链优先且不重复展开）、中/粤一字一音节、汉字读音不可知按 1 音符标 needs_review。精确 ASCII `+`、`-`、`br` 与 apostrophe 前缀使用官方 V2 特殊歌词语义，近似拼写保持原文并告警；token/per-note 返回 `semanticRole`/`semanticEvidence`，orphan 链与单独 apostrophe 要求人工复核。单元/音符数不匹配如实报告不自作主张。产出带 `expected.lyrics` 前置条件的统一 `apply` 信封（旧 `patchRequest` 保留一版并标注 deprecated）；变更超过单次 200 patch 上限时返回前 200 + `continuation` 工作流（警告 `PLAN_EXCEEDS_PATCH_CAP`）：提交成功会使 contextId 失效且 noteId 内嵌 contextId，后续批次无法预生成——commit → 重拍 `sv_snapshot_range` → 用相同参数重跑本工具，已应用音符自动 no-change，循环收敛到 no_change；显式 occurrenceId/startNoteId 仅在短期 continuation 身份记录证明 target UUID 相同且音符结构未漂移时重锚定（`STALE_SELECTOR_REANCHORED`），伪造、过期或漂移 selector 均拒绝；不承诺与宿主 G2P 一致 |
| `sv_analyze_phrase` | 只读乐理分析（纯内存）：时值加权音级直方图 × 24 个 Krumhansl-Kessler profile 的皮尔逊相关返回**排序**调性候选与次名差距（关系大小调/长属音歧义如实暴露，不隐藏）；音级与外音标记（升号拼写、自然小调，附各模式共有的 summary 直方图）；休止阈值乐句切分（climax/ambitus/休止）；音域/音程/节奏统计。官方 V2 特殊歌词状态机把精确 `br`、orphan `+/-` 与未校准的单独 apostrophe 排除出旋律推断，合法 continuation 链仍作为有音高旋律；`inputNoteCount`、`melodicNoteCount` 与 `excludedEvents` 明确说明每次排除，呼吸另以 `breathEvents`（`nominalPitch`）返回。另有四个 **opt-in** 和声语境 section：`metricalRoles`（逐音符小节/拍位置与 downbeat/strong/weak/offbeat 权重，含弱起检测）、`chordCandidates`（按小节或 `half_bar` 聚合音级，权重 = 窗内时值 × 节拍权重，匹配 12 种三/七和弦模板 × 12 根音，返回排序候选与 root/quality/score/`chordTonesPresent`/**`chordTonesAbsent`**（旋律根本没触及、纯属推测的和弦音）/`nonChordTones`/`runnerUpGap`）、`cadence`（句末与倒数第二音级 + 节拍位置 + 和弦窗口的启发式排序，每条候选带 rationale）、`tensionResolution`（导音解决/未解决、调外音解决/未解决、悬留式下行，每个事件同时给出前后 noteId、前后音级和实际半音运动）。**关键诚实边界：只观察得到单旋律**，因此每个 section 与每个 chord window 都声明 `evidenceScope:"melody_only"`——候选是与旋律相容的音集，绝不是对真实伴奏的观测，真实伴奏可能完全不同；歧义窗口/句末返回多个排序候选并标 `ambiguous`，`maxChordCandidates`/`maxCadenceCandidates` 下限为 2，从 schema 层面杜绝把最高分写成唯一事实；confidence 是排序差距而非概率。缺 meterMarks 时 `metricalRoles`/`chordCandidates` 如实报 `not_captured`，绝不假设 4/4。responseMode 实际生效：compact 只给汇总不展开逐音符，standard 大列表截断 100 条并警告，verbose 全量。全部 derived/heuristic，绝不冒充宿主事实 |
| `sv_style_profile` | 工程级风格统计聚合（纯内存只读）：对 1--8 个 range context 逐 target 报告旋律音符（官方精确 `br` 单独计数）的音域/音程直方图/节奏/休止、乐句长度分布、languageOverride 分布、Automation **控制点**统计（点数/min/max/mean/非默认占比/逐乐句 min-max——不是宿主插值后的可闻曲线）与可观测 vocalModeParams 键名（singer 身份仍 unobservable）；聚合层合并样本重算中位数（不平均中位数）、音程直方图求和（不跨 target 边界拼音程），并按调用方自供的 `label` 分组并排（verse/chorus 等段落标签绝不由服务推断——`sectionLabels:"caller_provided_not_inferred"`）。context 未 include automation/voiceParameters 的 target 对应剖面如实报 `not_captured` + 警告，不拿 0 冒充实测 |
| `sv_validate_lyrics_prosody` | 咬字/韵律校验器（纯内存只读，只报告不生成 patch——修复动作指向 sv_patch_notes/sv_align_lyrics/sv_restructure_notes）：breath（官方精确 `br` 带 language/phonemes override 或异常长换气）、specialLyricChains（统一状态机报告 `ORPHAN_PLUS`、`ORPHAN_PHONATION_CONTINUATION`、`SYLLABLE_CHAIN_GAP/OVERLAP`、单独 apostrophe 与可疑近似拼写，保留 noteIds/role/gap 证据）、japaneseMora（单音符多 mora、孤立小假名，确定性规则）、englishSyllables（元音簇音节数 vs 其后 "+" 续拍数，~85-90% 启发式）、languageConsistency（字符类别与 languageOverride 冲突、续拍残留 override）、stressAlignment（首音节重读近似 vs 拍强——无词典，只发 info 级 `confidence:"low"`，绝不报 error）、phonemeCoverage（旋律词音符的空音素；br/"-"/"+" 的空音素合法，无 processing 数据时如实 `not_captured`）。issues 按严重度+乐谱时间排序，standard 截断 100 并警告 |
| `sv_quantize_notes` | 无副作用量化规划器（纯内存只读）：onset 吸附到以小节边界为原点的网格（`1/4`…`1/32` 与 `1/8T`/`1/16T` 三连音，拍号变化处重锚），`strength` 线性插值、`swing` 奇数格后移（三连音网格拒绝 swing）、可选时值量化。确定性且不重排：同格碰撞的后音保留原位（`QUANTIZE_COLLISION`），量化引入的重叠撤销该 onset 变更（`OVERLAP_AFTER_QUANTIZE`，`quantizeDurations:true` 时改为收短前音）；**不提供 humanize**（随机微时移与确定性规划器契约冲突）；"br" 换气照常量化。产出带 expected onset/duration 前置条件的统一 `apply` 信封（旧 `patchRequest` 保留一版并标注 deprecated）；>200 patch 走与 sv_align_lyrics 相同的 continuation 收敛工作流（commit → 重拍快照 → 同参重跑，显式 occurrenceId 仅在短期身份记录证明同一 target UUID 时重锚定） |
| `sv_generate_harmony` | 调内和声规划器（纯内存只读，不创建轨道/组——先用 `sv_clone_track_from_template` 等准备目标组并重拍快照，使源与目标共享同一 range context）：旋律源音符（br 跳过）按自然音阶级映射三度/六度上下方，显式 key 或 K-S 检测（margin 过薄警告 `KEY_AMBIGUOUS` 并给出次名候选）；整数 occurrence pitch offset 会进入实际发声音高上的调性/register/声部交叉计算，写入请求与 `harmonyPitch` 使用目标本地 MIDI，`harmonySoundingPitch` 回显实际发声坐标；非整数 offset 因下游只接受整数 MIDI 音高而明确返回 `UNSUPPORTED_PITCH_OFFSET`；调外源音按最近调内音半音差近似并标 needsReview；register 越界先八度位移再跳过，位移导致声部交叉则 `VOICE_CROSSING_AVOIDED` 跳过；lyricsMode copy/sustain。目标既有 onset、时值、本地音高和歌词均一致的音符才视为 already_applied（收敛基础），同跨度不同内容列入 `TARGET_NOTE_CONFLICT` **绝不覆盖**。产出目标本地坐标的统一 `apply` 信封（`sv_restructure_notes` insert；旧 `restructureRequest` 保留一版并标注 deprecated）；>64 操作走 continuation 收敛工作流。`harmony.interval` 还接受广义 `{degree,direction,octaveOffset?}`（1--7 度含 unison、above/below、八度位移，`third/sixth` 是其别名）；`harmony.key.scale` 可显式指定 14 种音阶（七种调式 + harmonic/melodic minor + major/minor pentatonic + blues + whole-tone + chromatic，缺省 mode→ionian/aeolian），只接受调用者显式批准，K-S 绝不检测扩展调式；调外音如实返回 review warning 而非主观断言。和声好不好听 human_only |
...
术语说明：协议字段 `gestures[]` 保持不变；中文统一称“表现手法”，其中 pitch gesture
称“音高变化手法”，表示一段有范围和形状的参数变化，不指面部表情或肢体动作。
...
- `svcopilot://guide/music-workflows`：面向 LLM 的工作流指南目录——全局规则（身份、context 生命周期、写入安全、证据分级、人类门、能力阻塞、错误分类）加 9 个 recipe 摘要。
- `svcopilot://guide/music-workflows/{recipe}`：单个 recipe 全文，含每步工具、最小请求模板、必要 `include` 字段、可接受与不可重试状态、共享 target 与 Undo 影响、人类门和 capability-blocked 分支。可用 recipe：`inspect_project`、`analyze_vocal_phrase`、`align_and_commit_lyrics`、`plan_and_commit_expression`、`plan_and_commit_pitch`、`quantize_notes`、`generate_harmony`、`verify_after_edit`、`audition_for_human`。
