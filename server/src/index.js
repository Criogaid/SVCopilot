...
export const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Capture one editable musical-range context with occurrence and note identities, including notes sustained across the range start. A single host lease can include notes plus host-declared group scale metadata, voice parameters, Automation, computed pitch, attributes, processing (including per-note host G2P phoneme strings), tempo/meter maps, and mixer state including host FX parameters when observable. PitchControlPoint items report runtime isTemporary when observable; that diagnostic never becomes identity. Decimal or rational beats are converted exactly with host SV.QUARTER. Independent response budgets page already-captured pure data through cursor without rereading the host; global capture limits are published in svcopilot://capabilities. snapshotToken is a content hash, not a host revision; sinceToken still reads and hashes before returning no_change.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
